        ...
        with Horizontal():
            with Vertical():
                ...
                with Container(id="input-container"):
                    self.input = Input(
                        placeholder="> Type here... (press Ctrl+T to toggle thoughts, /attach to upload image)", 
                        id="chat-input"
                    )
                    yield self.input
...
💡 [dim]Gợi ý cho bạn:[/dim]
  ...
  [#a166ab]2.[/] Tạo module đăng nhập với JWT
  [#a166ab]3.[/] /mode fast Tối ưu hóa hệ thống
"""
        # Create a large vertical padding before the logo to push it down
        padding = "\n" * 5
    ...
    def handle_agent_message(self, msg):
        content = str(msg.payload.get("task") or msg.payload.get("result") or msg.payload.get("error") or msg.payload)
        ...
        if msg.type.value == "task_result":
            ...
            if self.show_thoughts:
                self.chat_log.write(f"[dim]{msg.sender}[/] ➔ [dim]{msg.recipient}[/]: {short_content}")
            self.session_log.update(f"Status: [bold #a166ab]{msg.sender}[/] is working")
            
        self.iteration += 1

    @work(thread=True)
    def run_nexus_team(self, query: str):
        self.call_from_thread(self.chat_log.write, f"[bold #00f2fe]> {query}[/]\n")
        try:
            self.team.run(query)
        except Exception as e:
            self.call_from_thread(self.chat_log.write, f"[bold #ff9a9e]System Error:[/] {e}\n")
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if query.startswith("/attach"):
            ...
            self.team.memory.remember(f"User attached file: {query[8:]}", category="file_attachment")
            return
    ...
    def action_toggle_plan(self) -> None:
        self.mode = "PLAN"
        self.chat_log.write(f"[italic #00f2fe]Switched to PLAN mode[/]\n")
        
    def action_toggle_fast(self) -> None:
        self.mode = "FAST"
        self.chat_log.write(f"[italic #00f2fe]Switched to FAST mode[/]\n")

    @work(thread=True)
    def action_index_codebase(self) -> None:
        self.call_from_thread(self.chat_log.write, "[bold #a166ab]Indexing codebase with GitNexus...[/]\n")
        self.call_from_thread(self.session_log.update, "Status: [bold #a166ab]Indexing...[/]")
        
        repo_intel = RepoIntelligence(self.config.project_dir)
        output = repo_intel.analyze()
        
        if "Error" in output:
            self.call_from_thread(self.chat_log.write, f"[bold #ff9a9e]Indexing Failed:[/] {output}\n")
        else:
            self.config.gitnexus = True # Enable it automatically after success
            self.call_from_thread(self.chat_log.write, f"[bold #A8D5BA]Indexing Complete![/] Context files (AGENTS.md) updated.\n")
            self.call_from_thread(self.chat_log.write, f"[dim]Nexus AI will now use the Repo Map to save tokens.[/]\n")
